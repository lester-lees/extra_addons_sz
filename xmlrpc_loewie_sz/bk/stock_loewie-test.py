...
class stock_picking(osv.osv):
    ...
    _peer_url = "http://192.168.0.100:8069/xmlrpc/object"	
    _peer_redirect = 'http://192.168.0.100:8069/web?#id=%d&view_type=form&model=stock.picking&action=370&active_id=%d'	
    ...
    def view_remote_erp_picking(self, cr, uid, ids, context=None):
        ...
        for po in self.browse(cr, uid, ids, context=context):
            if po.remote_picking_id > 0 :	 
                ids = my_proxy.execute(self._db, 1, self._pwd, 'stock.picking', 'action_confirm', po.remote_picking_id)				
