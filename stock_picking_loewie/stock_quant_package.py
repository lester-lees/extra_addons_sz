...
class stock_move(osv.osv):
    ...
    _columns = {
        ...
        'state': fields.selection([('draft', 'New'),
                                   ...
                                   ('assigned', 'Available'),								   
                                   ('done', 'Done'),
                                   #('waiting_financial', u'财务已审核'),	# 此处 waiting_financial 与 财务已审核 ， 词不达意								   
    ...
    def action_assign(self, cr, uid, ids, context=None):
        ...
        operations = set()
        for move in self.browse(cr, uid, ids, context=context):
            if move.state not in ('confirmed', 'waiting', 'assigned', 'waiting_financial'):  #  , 'waiting_financial'
                continue
            ...
            if move.product_id.type == 'consu':
                ...
                if move.state in ['waiting','waiting_financial'] and not ancestors:  # ,'waiting_financial'
...
class stock_picking(osv.osv):
    ...
    def _state_get(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for pick in self.browse(cr, uid, ids, context=context):
            ...
            order = {'confirmed': 0, 'waiting': 1,  'waiting_financial': 2, 'assigned': 3}   #  'waiting_financial',
            order_inv = {0: 'confirmed', 1: 'waiting', 2: 'waiting_financial', 3: 'assigned'}  #  'waiting_financial',
    ...
    def emily_known(self, cr, uid, ids, context=None):	
        for pick in self.pool.get('stock.picking').browse(cr, uid, ids, context=context):
            ...
            statement = "update stock_move set state='waiting_financial' where picking_id=%d; update stock_picking set state='waiting_financial' where id=%d" % (pick.id,pick.id)
            cr.execute(statement)			
			
            #for x in pick.move_lines :
            #    x.state = 'waiting_financial'
				
            #pick.state = 'waiting_financial'				
    ...
    def force_assign(self, cr, uid, ids, context=None):
        ...
        for pick in self.browse(cr, uid, ids, context=context):
            move_ids = [x.id for x in pick.move_lines if x.state in ['confirmed', 'waiting','waiting_financial']]    #  ,'waiting_financial'
    ...
    def create_negative_so(self, cr, uid, ids, context=None ):		
        ...
        if this.picking_type_id.id not in [1,20] :
            vals['client_order_ref'] = u'冲减销售单, 原仓库单:' + this.name
            vals.pop('return_pick')			
