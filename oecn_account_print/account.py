...
class account_periodly(osv.osv):
    ...
    _columns = {
        ...
        'parent_id':fields.many2one('account.account','父科目',readonly=True),		
        'account_id':fields.many2one('account.account','科目',readonly=True),
        'partner_id':fields.many2one('res.partner','业务伙伴',readonly=True),	
        'section_id':fields.many2one('crm.case.section','销售分组',readonly=True),		
    ...
    def init(self, cr):
        ...
        cr.execute("""
            ...
            where l.state != 'draft'
            group by p.id, a.parent_id, l.partner_id, rp.section_id, l.account_id, p.fiscalyear_id, p.date_start, am.company_id
            )
        """)
