...
class loewieec_shop(osv.osv):
    ...
    def create_order(self, cr, uid, ids, trade, context=None):	
        ...
        for order in trade:
            ...
            for line in lines:
                line['tid'] = str(order['tid'])			
                order_lines.append(line)
				
        for order_line in order_lines:  
            tid = order_line.get('tid')		
            line_vals = {			
            ...
            product_tmalljd_ids = self.pool.get('product.tmalljd').search(cr, uid, [('ec_sku_id','=',order_line.get('sku_id'))], context = context )
            
            #如果没有匹配到产品，报同步异常  coe_lines
            if not product_tmalljd_ids:
                syncerr = u"订单导入错误: 匹配不到商品。tid=%s, 商品【%s】, outer_iid=%s, num_iid=%s, outer_sku_id=%s, sku_id=%s " % ( tid, order_line.get('title', ''), order_line.get('outer_iid', ''), order_line.get('num_iid', ''),  order_line.get('outer_sku_id', ''), order_line.get('sku_id', '') )
                self.pool.get('loewieec.error').create(cr, uid, {'name':syncerr, 'shop_id':shop.id }, context = context )
                return False
    ...
    def search_orders_by_created_time(self, cr, uid, ids, date_start = None, date_end = None, status = 'WAIT_SELLER_SEND_GOODS', context=None):
        port = 80
        shop = self.browse(cr, uid, ids[0], context = context)
        partner_id = shop.partner_id.id		
        setDefaultAppInfo(shop.appkey, shop.appsecret)
    ...
    def remove_duplicate_tmijdi_no(self, cr, uid, ids, orders, shop, context=None):
        statement = "select tmi_jdi_no from sale_order_line where state != 'cancel' group by tmi_jdi_no"
        ...
        if len(last_log) > 0:
            shop.last_log = ",".join(last_log) + u" - 以上电商单号ERP中已存在,所以此次没有导入。"
        return res			
