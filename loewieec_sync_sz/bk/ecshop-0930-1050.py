...
class loewieec_shop(osv.osv):
    ...
    def create_order_from_excel(self, cr, uid, ids, coe_lines, product_lines, tmijdi_fname, context=None):	
        ...
        for key in coe_lines:
            ...
            coe_id = coe_obj.search(cr, uid, [('name','=',coe_line["express_no"])], context=context)
            ...
            note += u"TMJD NO:%s, COE:%s, 姓名:%s, 电话:%s, 省份:%s, 城市%s, 收货地址:%s, 邮编:%s;"	% (coe_line["tmi_jdi_no"], coe_line["express_no"], coe_line["name"],coe_line["mobile"],coe_line["state"],coe_line["city"],coe_line["address"],coe_line["zip"] )
            note += chr(10)	
    ...
    def import_orders_from_excel(self, cr, uid, ids, context=None):
        ...
        while row_start <= highest_row and ws.cell(row=row_start,column=2).value :		
            ...
            if tmi_jdi_no and express_no and name and mobile and address: 		
                ...
                coe_lines[last_tmi_jdi_no] = {"id":1, "tmi_jdi_no": tmi_jdi_no, "express_no":express_no, "name":name, "mobile":mobile, "address":address, "city":city, "state":state, "zip":zip}				
    ...
    def search_orders_by_created_time(self, cr, uid, ids, context=None, status = []):
        ...
        filtered_orders = self.search_orders_seller_memo(cr, uid, ids, context,orders)		
        order_id = self.create_order(cr, uid, ids, filtered_orders, context = context )
    ...
    def import_express_no(self, cr, uid, ids, context=None):
        ...
        while row_start <= highest_row and ws.cell(row=row_start,column=0).value :		
            ...
            coe_lines[tmi_jdi_no] = {"tmi_jdi_no": tmi_jdi_no, "name":express_no, "name":name, "mobile":mobile, "address":address, "city":city, "state":state, "zip":zip}

            note += u"TMJD NO:%s, COE:%s, 姓名:%s, 电话:%s, 省份:%s, 城市%s, 收货地址:%s, 邮编:%s;"	% (tmi_jdi_no, express_no, name,mobile,state,city,address,zip )
            note += chr(10)				
